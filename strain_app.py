            ...
            for i, row in edited_df.iterrows():
                try:
                    ...
                    with st.expander(f"📊 {row['파일명']} 상세 분석"):
                        ...
                        with col1:
                            # 2D 패턴 상하 반전 (물리적 픽셀 매트릭스 전체를 뒤집음)
                            img_flipped = np.flipud(img)
                            fig2d, ax2d = plt.subplots()
                            im = ax2d.imshow(np.log1p(np.clip(img_flipped, 0, None)), cmap='jet', origin='lower')
                        ...
                        with col2:
                            ...
                            ax1d.axvline(q_exp, color='r', linestyle='--', label=f'Peak: {q_exp:.3f}')
                            ax1d.set_title(f"Peak: {q_exp:.4f} / Strain: {strain*100:.3f}%")
                            ax1d.legend()
